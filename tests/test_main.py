...
class Test_find_start_repeat_kmers(unittest.TestCase):
	...
	def test_fake1(self):
		...
		with open("fake1_61mers.hgram", 'r') as f:
			...
			self.assertTrue(73 <= grin_main.find_start_repeat_kmers(hist_dict) <= 79)
	...
	def test_celegans(self):
		...
		with open("celegans_61mers.hgram", 'r') as f:
			hist_dict = grin_main.create_hist_dict(f)
			self.assertTrue(27 <= grin_main.find_start_repeat_kmers(hist_dict) <= 33)

	def test_yeast(self):

		with open("yeast_51mers.hgram", 'r') as f:
			hist_dict = grin_main.create_hist_dict(f)
			self.assertTrue(66 <= grin_main.find_start_repeat_kmers(hist_dict) <= 72)
