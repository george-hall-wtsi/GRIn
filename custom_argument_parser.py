...
class CustomParser(argparse.ArgumentParser):
	...
	def generate_usage_message(self):
		...
		store_str += "[-E single-error-cutoff] [-e error-cutoff [error-cutoff ...]] "
	...
	def generate_help_message(self):
		...
		store_str += "\t-E, --single-error-cutoff  manual cutoff of error peak, applied to "
		store_str += "all files\n"
		...
		store_str += "equal the number of input files\n"
		store_str += "\t* if specified, --upper-bound, --single-error-cutoff, and "
		store_str += "--error-cutoffs must be integers\n"
		store_str += "\t* --single-error-cutoff, --error-cutoffs, and --ignore-error are "
		store_str += "mutually exclusive\n\n"
